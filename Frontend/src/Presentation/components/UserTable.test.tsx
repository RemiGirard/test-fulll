...
const USER_LIST: User[] = [
  { id: '1', login: 'first', avatarUrl: '' },
  { id: '2', login: 'second', avatarUrl: '' },
];

describe('UserTable', () => {
  it('renders loading state', () => {
    render(<UserTable userList={[]} RenderUser={RenderUser} isLoading={true} />);
    expect(screen.getByText(/loading/i)).toBeTruthy();
  });

  it('renders list of users with RenderUser', () => {
    render(<UserTable userList={USER_LIST} RenderUser={RenderUser} isLoading={false} />);
  ...
  it('renders nothing when no users and not loading', () => {
    render(<UserTable userList={[]} RenderUser={RenderUser} isLoading={false} />);
